...
//
// Full-bleed cover with the title/meta overlaid on a translucent gradient
// (not a separate solid bar below the image) — the whole card is one Link, so
// there's no separate "open" button either.
export function ActiveBlockCard({ block }: { block: Block }) {
  ...
  return (
    <Link
      ...
      className="group relative block aspect-[4/5] overflow-hidden rounded-2xl border border-white/[0.08] transition-colors hover:border-white/[0.16]"
    >
      <BlockCover src={cover} type={block.blockType} />

      {/* Translucent gradient the title/meta sit on — lets the cover read
          through clearly instead of a solid bar. */}
      <span className="pointer-events-none absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-black/55 via-black/20 to-transparent" />
      ...
      {/* Title + meta, overlaid at the bottom of the cover. */}
      <div className="absolute inset-x-0 bottom-0 p-3.5">
        <p className="truncate text-[14px] font-semibold text-white drop-shadow-[0_1px_4px_rgb(0_0_0/0.55)]">
          {block.title}
        </p>
        {isPending ? (
          <p className="mt-0.5 inline-flex items-center gap-1.5 text-[12px] text-[#F5C95B]/90 drop-shadow-[0_1px_3px_rgb(0_0_0/0.6)]">
            <Clock size={12} /> Request sent {sentAgo(block.pendingSince)}
          </p>
        ) : (
          <p className="mt-0.5 inline-flex items-center gap-1.5 text-[12px] text-white/80 drop-shadow-[0_1px_3px_rgb(0_0_0/0.6)]">
